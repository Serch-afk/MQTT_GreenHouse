 ...
 */
void mqtt_freertos_run_thread(bool* wifi_status)
{
    LOCK_TCPIP_CORE();
    ...
    if (sys_thread_new("app_task", app_thread, wifi_status, APP_THREAD_STACKSIZE, APP_THREAD_PRIO) == NULL)
    {
        LWIP_ASSERT("mqtt_freertos_start_thread(): Task creation failed.", 0);
    }
}
